...
[TestFixture]
public class WhenOwnsManyMutateWithOwnsOneMutate : InterceptorUpdateTestBase
{
    private string _newPhone = null!;
    ...
    {
        _newPhone = Phone.Random().Value;
    }

    protected override void OnUpdate(User user)
    {
        user.MutateContactPhoneValue(1, _newPhone);
    }
    ...
    protected override IDictionary<string, object?> ExpectedValues()
    {
        ...
        values["Contacts"] = new[] { contacts.ElementAt(0), new Contact(new Phone(_newPhone), InStorage!.Contacts.ElementAt(1).AdditionalPhone?.Clone()).ToDictionary() };
        return values;
    }
}